...
// 9. Secure Key Management (Admin Only)
export const saveSecureKeys = async (keys: string[]) => {
    try {
        const sanitized = sanitizeForFirestore({ apiKeys: keys });
        await set(ref(rtdb, 'secure_keys'), sanitized);
        await setDoc(doc(db, "config", "secure_keys"), sanitized);
    } catch (e) { console.error("Error saving secure keys:", e); }
};

export const getSecureKeys = async (): Promise<string[]> => {
    try {
        // Try Firestore (More secure if rules set)
        const docSnap = await getDoc(doc(db, "config", "secure_keys"));
        if (docSnap.exists() && docSnap.data().apiKeys) {
            return docSnap.data().apiKeys;
        }
        
        // Try RTDB
        const snap = await get(ref(rtdb, 'secure_keys'));
        if (snap.exists() && snap.val().apiKeys) {
            return snap.val().apiKeys;
        }
        return [];
    } catch (e) { 
