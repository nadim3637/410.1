...
import { getChapterData, getCustomSyllabus, getSecureKeys, rtdb } from "../firebase";
...
const getAvailableKeys = async (): Promise<string[]> => {
    ...
    // 2. Try Secure Keys from Firebase
    try {
        const secureKeys = await getSecureKeys();
        if (secureKeys && Array.isArray(secureKeys)) {
    ...
    } catch (e) { console.error("Failed to fetch secure keys", e); }
...
export const executeWithRotation = async <T>(
    ...
    source: 'ADMIN' | 'STUDENT' = 'STUDENT'
): Promise<T> => {
    // 1. AI USAGE SPLIT CHECK (For Students Only)
    if (source === 'STUDENT') {
        let splitRatio = 0.2; // Default 20% for Students
        try {
            const s = localStorage.getItem('nst_system_settings');
            if (s) {
                const settings = JSON.parse(s);
                if (settings.aiUsageSplit !== undefined) {
                    splitRatio = (100 - settings.aiUsageSplit) / 100; // e.g. 80 -> 20% for students
                }
            }
        } catch(e) {}

        if (Math.random() > splitRatio) {
            // Simulated Rejection based on quota
            throw new Error("AI Pilot is currently prioritizing content generation. Please try again later.");
        }
    }
    ...
    if (keys.length > 0) {
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            try {
                ...
                const usageRef = ref(rtdb, `api_usage/${keyId}`);
                runTransaction(usageRef, (current) => (current || 0) + 1);
...
export const translateToHindi = async (content: string, isJson: boolean = false): Promise<string> => {
    const prompt = `
...
export const fetchLessonContent = async (
  ...
  if (!forceRegenerate) {
      ...
      if (adminContent) {
          // DRAFT CHECK (Admin Override)
          if (adminContent.status === 'DRAFT' && source !== 'ADMIN') {
               return {
                  id: Date.now().toString(),
                  title: chapter.title,
                  ...
                  dateCreated: new Date().toISOString(),
                  subjectName: subject.name,
                  isComingSoon: true // Show Coming Soon
              };
          }
  ...
  if (dualGeneration && (type === 'NOTES_PREMIUM' || type === 'NOTES_SIMPLE')) {
      const [premiumResult, freeResult] = await Promise.all([
          generateNotes(true),
          generateNotes(false)
      ]);
      ...
      return {
          ...
          content: premiumResult.text, // Default to Premium
          type: 'NOTES_PREMIUM',
          dateCreated: new Date().toISOString(),
          subjectName: subject.name,
          
          schoolPremiumNotesHtml: syllabusMode === 'SCHOOL' ? premiumResult.text : undefined,
          competitionPremiumNotesHtml: syllabusMode === 'COMPETITION' ? premiumResult.text : undefined,
          schoolPremiumNotesHtml_HI: syllabusMode === 'SCHOOL' ? premiumResult.hindiText : undefined,
          competitionPremiumNotesHtml_HI: syllabusMode === 'COMPETITION' ? premiumResult.hindiText : undefined,

          schoolFreeNotesHtml: syllabusMode === 'SCHOOL' ? freeResult.text : undefined,
          competitionFreeNotesHtml: syllabusMode === 'COMPETITION' ? freeResult.text : undefined,
